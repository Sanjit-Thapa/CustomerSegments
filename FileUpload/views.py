...
def Home(request):
         ...
         if request.method == 'POST':
             if request.POST.get('action') == 'download_single_pdf':
                 ...
                 # Convert plot URLs to absolute file paths or base64 for wkhtmltopdf
                 if section == 'plots':
                     pdf_context['section_data'] = []
                     for plot in results.get('plots', []):
                         if not plot.startswith('data:image'):
                             file_path = os.path.join(settings.MEDIA_ROOT, plot.lstrip(settings.MEDIA_URL))
                             if os.path.exists(file_path):
                                 pdf_context['section_data'].append(file_path)
                             else:
             ...
             if csv_file_form.is_valid():
                 ...
                 # Adjust plots to include full URLs for UI display
                 if 'plots' in results:
                     results['plots'] = [
                         f"{settings.MEDIA_URL}{plot}" if not plot.startswith('data:image') else plot
                         for plot in results.get('plots', [])
                     ]
